...
describe('verifySignature', () => {
    it('returns false for empty', () => {
        expect(Civitas.verifySignature("")).toBe(false);
    })
    it('returns false for random', () => {
        expect(Civitas.verifySignature("abc")).toBe(false);
    })
    it('returns false for a dict', () => {
        expect(Civitas.verifySignature({"abc": "def"})).toBe(false);
    })
    it('returns false for invalid hash', () => {
        expect(Civitas.verifySignature(EXAMPLE_INVALID_HASH)).toBe(false);
    })
    it('returns false for invalid public key', () => {
        expect(Civitas.verifySignature(EXAMPLE_INVALID_PUBLIC_KEY)).toBe(false);
    })
    it('returns false for invalid signature', () => {
        expect(Civitas.verifySignature(EXAMPLE_INVALID_SIGNATURE)).toBe(false);
    })
    it('returns true for valid', () => {
        expect(Civitas.verifySignature(EXAMPLE_VALID)).toBe(true);
    })
});

describe('Civitas.getHistory', () => {
    beforeEach(() => {
    ...
    it('should call fetch with limit=100 if limit is undefined', async () => {
        ...
        // Act
        await Civitas.getHistory(mockApiKey);

        // Assert
        expect(global.fetch).toHaveBeenCalledTimes(1);
        expect(global.fetch).toHaveBeenCalledWith(
            'https://api.arathiel.com/v1/verified/chat/completions?limit=100',
    ...
    it('should call fetch with limit=100 if limit is 0 or negative', async () => {
        ...
        // Act
        await Civitas.getHistory(mockApiKey, 0);
        await Civitas.getHistory(mockApiKey, -5);
    ...
    it('should call fetch with provided limit if limit is a positive number', async () => {
        ...
        // Act
        await Civitas.getHistory(mockApiKey, mockLimit);

        // Assert
        expect(global.fetch).toHaveBeenCalledTimes(1);
        expect(global.fetch).toHaveBeenCalledWith(
            `https://api.arathiel.com/v1/verified/chat/completions?limit=${mockLimit}`,
            expect.objectContaining({
                method: 'GET',
                headers: {
                    Accept: 'application/json',
    ...
    it('should call fetch without invalid filter', async () => {
        ...
        // Act
        await Civitas.getHistory(mockApiKey, mockLimit, null, "invalidFilter");
    ...
    it('should call fetch with provided filter mine', async () => {
        ...
        // Act
        await Civitas.getHistory(mockApiKey, mockLimit, null, "mine");

        // Assert
        expect(global.fetch).toHaveBeenCalledTimes(1);
        expect(global.fetch).toHaveBeenCalledWith(
            `https://api.arathiel.com/v1/verified/chat/completions?limit=${mockLimit}&filter=mine`,
            expect.objectContaining({
                method: 'GET',
                headers: {
                    Accept: 'application/json',
    ...
    it('should call fetch with provided filter all', async () => {
        ...
        // Act
        await Civitas.getHistory(mockApiKey, mockLimit, null, "all");

        // Assert
        expect(global.fetch).toHaveBeenCalledTimes(1);
        expect(global.fetch).toHaveBeenCalledWith(
            `https://api.arathiel.com/v1/verified/chat/completions?limit=${mockLimit}&filter=all`,
            expect.objectContaining({
                method: 'GET',
                headers: {
                    Accept: 'application/json',
    ...
    it('should return JSON data when response is ok', async () => {
        ...
        // Act
        const result = await Civitas.getHistory(mockApiKey, 20);
    ...
    it('should throw an error when response is not ok', async () => {
        ...
        // Act & Assert
        await expect(Civitas.getHistory(mockApiKey, 20)).rejects.toThrow(
    ...
    it('should call fetch with cursor if cursor is defined', async () => {
        ...
        // Act
        await Civitas.getHistory(mockApiKey, 100, "mockCursor");
...
describe('Civitas.getByHash', () => {
    ...
    it('should call fetch with correct url and hash', async () => {
        ...
        // Act
        await Civitas.getByHash(mockApiKey, "mockHash");
