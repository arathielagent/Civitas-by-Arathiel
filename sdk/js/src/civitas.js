...
export default class Civitas {
     ...
     *
     * @param {string} apiKey - Arathiel API Key
    ...
    static getHistory(apiKey, limit, cursor, filter) {
        ...
        let url = 'https://api.arathiel.com/v1/verified/chat/completions?limit=' + limit
        if (cursor) {
    ...
    static getByHash(apiKey, hash) {
        let url = 'https://api.arathiel.com/v1/verified/chat/completions/' + hash
        return fetch(url, {
            method: 'GET',
            headers: {
                'Accept': 'application/json',
