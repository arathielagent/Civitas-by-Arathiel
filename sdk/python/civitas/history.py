...
BASE_URL = "https://api.arathiel.com"


class ArathielChatHistory(BaseModel):
    request: Dict
...
def get_history(
    arathiel_api_key: str, limit: int = 10, cursor: str = None, filter: str = "all"
) -> List[ArathielChatHistory]:
    if filter not in ["all", "mine"]:
        raise ArathielError("Incorrect filter given, valid values are: 'all', 'mine'")

    query_params = {"limit": limit, "filter": filter}
    if cursor:
        query_params["cursor"] = cursor

    return _make_request(arathiel_api_key, query_params)


def get_by_hash(arathiel_api_key: str, hash: str) -> Optional[ArathielChatHistory]:
    try:
        url = urljoin(BASE_URL, f"/v1/verified/chat/completions/{hash}")
        headers = {"accept": "application/json", "Authorization": arathiel_api_key}
...
def _make_request(
    arathiel_api_key: str, query_params: Optional[Dict]
) -> List[ArathielChatHistory]:
    try:
